...
| URL/SSRF | HTTPS-only, no credentials, private/reserved IP and local hostname rejection; IPFS through configured gateway | Unsafe scheme/private host rejected |
...
| Analytics disclosure | Per-event property allowlist, primitive-only bounded values, wallet/URL rejection, no-op on validation failure | Nested fields and wallet-shaped values rejected |
